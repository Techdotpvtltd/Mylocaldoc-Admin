...
class FirestoreApi {
  ...
  postANode({required String path, required Map<String, dynamic> data}) async {
  ...
  checkANode({required String path}) async {
    try {
  ...
  deleteANode({required String path}) async {
    try {
  ...
  fetchANode({required String path}) async {
    MyResponse response = MyResponse();
    try {
      final event = await firebaseRef.doc(path).get();
      if (event.exists) {
  ...
  fetchCollection({required String path, Map<String, dynamic>? filter}) async {
